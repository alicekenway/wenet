...
AsrResult BuildAsrResult(const std::vector<DecodedHypothesis>& hyps,
  ...
  for (const DecodedHypothesis& hyp : hyps) {
    ...
    for (const DecodedEntity& source : hyp.entities) {
      ...
      entity.slot_token = source.slot_token;
...
FlashlightAsrStream::FlashlightAsrStream(
    std::shared_ptr<FlashlightAsrResources> shared,
    FlashlightDecoderResourcePtr slot_decoder_resource)
    : shared_(std::move(shared)),
      slot_decoder_resource_(std::move(slot_decoder_resource)) {}
...
Status FlashlightAsrStream::EnsureInitialized() {
  ...
  try {
    impl_ = std::make_unique<Impl>(
        shared_, slot_decoder_resource_ ? slot_decoder_resource_
                                        : shared_->decoder_resource);
    impl_->AddDebugLog("stream_init");
    if (slot_decoder_resource_) {
      const auto& contacts = slot_decoder_resource_->DynamicSlots();
      impl_->AddDebugLog("runtime_slot_context forms=" +
                         std::to_string(contacts ? contacts->Forms().size()
                                                  : 0));
      impl_->AddDebugLog("multi_lm_slot_decoder=true");
    }
...
Status FlashlightAsrStream::EmitPartialIfAvailable() {
  ...
  auto partial_or = impl_->decoder->PartialResults(shared_->config.nbest);
  if (!partial_or.ok()) return impl_->RecordStatus(partial_or.status());
  hyps = std::move(partial_or).value();
  if (hyps.empty()) {
    return Status::Ok();
  }
...
Status FlashlightAsrStream::FinalizeIfReady() {
  ...
  std::vector<DecodedHypothesis> hyps;
  auto hyps_or = impl_->decoder->Finalize(shared_->config.nbest);
  if (!hyps_or.ok()) return impl_->RecordStatus(hyps_or.status());
