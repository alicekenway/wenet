...
const fl::lib::text::TrieNode* Child(
...
bool HasChildren(const fl::lib::text::TrieNode* base,
                 const fl::lib::text::TrieNode* overlay) {
  return (base && !base->children.empty()) ||
         (overlay && !overlay->children.empty());
}

bool HasLabels(const fl::lib::text::TrieNode* base,
               const fl::lib::text::TrieNode* overlay) {
  return (base && !base->labels.empty()) ||
...
MultiTrieLexiconDecoder::MultiTrieLexiconDecoder(
    ...
    : options_(std::move(options)),
      ...
      runtime_lookahead_(std::move(runtime_lookahead)),
      lm_(std::move(lm)),
      sil_(sil),
      blank_(blank),
      unk_(unk),
  ...
  if (!base_lexicon_ || !overlay_lexicon_ || !runtime_lookahead_ || !lm_) {
    throw std::invalid_argument(
        "multi-trie decoder requires both tries, lookahead, and LM");
  }
}

bool MultiTrieLexiconDecoder::IsRoot(
    const MultiTrieLexiconDecoderState& state) const {
  return state.base_lex == base_lexicon_->getRoot() &&
...
double MultiTrieLexiconDecoder::Lookahead(
  ...
  if (state.base_lex) {
    const auto override = runtime_lookahead_->find(state.base_lex);
    score = override == runtime_lookahead_->end()
                ? state.base_lex->maxScore
                : override->second;
  }
...
void MultiTrieLexiconDecoder::decodeBegin() {
  ...
  hyp_[0].emplace_back(
      0.0, lm_->start(false), base_lexicon_->getRoot(),
  ...
  for (int t = 0; t < frames; ++t) {
    ...
    for (const MultiTrieLexiconDecoderState& previous :
      ...
      for (int rank = 0;
        ...
        const fl::lib::text::TrieNode* base_child =
            Child(previous.base_lex, token);
        const fl::lib::text::TrieNode* overlay_child =
            Child(previous.overlay_lex, token);
        if (!base_child && !overlay_child) continue;
        ...
        auto add_words = [&](const fl::lib::text::TrieNode* node) {
          if (!node) return;
          for (int label : node->labels) {
            if (previous_is_root && previous.token == token) continue;
            ...
            fl::lib::text::candidatesAdd(
                ...
                score + options_.lmWeight * word_lm_score +
                    options_.wordScore,
                word_lm_state, base_lexicon_->getRoot(),
                overlay_lexicon_->getRoot(), &previous, token, label, false,
                previous.emittingModelScore + am_score,
                previous.lmScore + word_lm_score);
          }
        };
        add_words(base_child);
        add_words(overlay_child);

        if (!HasLabels(base_child, overlay_child) &&
          ...
          fl::lib::text::candidatesAdd(
              ...
              lm_state, base_lexicon_->getRoot(), overlay_lexicon_->getRoot(),
