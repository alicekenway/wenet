# WeNet SDK 0.0.15: Unified Linux x86-64 and Android ARM64 Build

SDK 0.0.15 retains the flexible multi-LM decoder and optional inverse text
normalization.

This version keeps the 0.0.13 startup improvements and changes runtime contact
contexts to share the package-sized token/word dictionaries, lexicon metadata,
fixed KenLMs, and immutable base trie. A context owns only its dynamic contact
...
inside a single beam search. The no-contact path still uses Flashlight's
original single-trie decoder.

The base trie also shares a lazily prepared sparse lookahead table. It records
only nodes whose strict runtime-contact max differs from the original trie
score, preserving the former combined-trie pruning scores without rebuilding
or copying the base trie for every contact context.
...
Linux retains all configured backends. Android compiles the production
Flashlight path only. Both release targets are pinned to ONNX Runtime 1.16.3.
