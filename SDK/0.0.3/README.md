# WeNet SDK 0.0.3: Flashlight + KenLM CTC Decoder
...
`ASR_SDK_ENABLE_LEGACY_WFST=ON`, but the new 0.0.3 package does not require
`TLG.fst`.
...
0.0.3 expects ONNX Runtime `1.25.1` under this SDK tree:
...
```bash
ORT_VERSION=1.25.1
SDK_ROOT=/home/jinyang_wang/Dev/ASR/ASR_wenet/wenet/SDK/0.0.3
...
```bash
cd /home/jinyang_wang/Dev/ASR/ASR_wenet/wenet/SDK/0.0.3
...
`sil_token`, `unk_word`, and `sample_rate`.
Use `feature_type=kaldi` for standard sherpa/icefall Zipformer CTC models; the
default `whisper` mode is kept for packages that were built with the earlier
SDK frontend.
...
```bash
wenet/SDK/0.0.3/build/zipformer_ctc_flashlight_main \
  --model test/0.0.3/model_flashlight/model.onnx \
  --tokens test/0.0.3/model_flashlight/tokens.txt \
  --words test/0.0.3/model_flashlight/words.txt \
  --lexicon test/0.0.3/model_flashlight/lexicon.txt \
  --lm test/0.0.3/model_flashlight/lm.bin \
  --mapping test/0.0.3/model_flashlight/output_mapping.txt \
...
```bash
wenet/SDK/0.0.3/build/asr_stream_file \
  --model_dir test/0.0.3/model_flashlight \
  --wav model/sherpa-onnx-streaming-zipformer-ctc-zh-2025-06-30/test_wavs/0.wav \
  --print_partial false
```
...
```bash
wenet/SDK/0.0.3/build/asr_package_eval \
  --model_dir test/0.0.3/model_flashlight \
  --decode_mode lm \
  ...
  --output_json test/0.0.3/package_eval/output.jsonl

python3 wenet/SDK/0.0.3/tools/summarize_asr_package_eval.py \
  --input_json test/0.0.3/package_eval/output.jsonl \
  --summary test/0.0.3/package_eval/summary.txt
```

Use `--decode_mode greedy` to evaluate AM-only CTC greedy decoding without
loading the lexicon or KenLM.

See `tools/README.md` for field definitions and tokenization behavior.
