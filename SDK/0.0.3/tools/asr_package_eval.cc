...
struct GreedyContext {
  ...
  std::shared_ptr<Zipformer2CtcOnnxResource> acoustic_resource;
...
std::vector<std::string> SplitUtf8CharsNoWhitespace(const std::string& text) {
  std::vector<std::string> tokens;
...
std::vector<std::string> ErrorUnits(const std::string& ref,
                                    const std::string& hyp) {
  if (HasAsciiWhitespace(ref) || HasAsciiWhitespace(hyp)) {
    return SplitWhitespace(ref);
  }
  return SplitUtf8CharsNoWhitespace(ref);
}

std::vector<std::string> HypUnitsLikeRef(const std::string& ref,
                                         const std::string& hyp) {
  if (HasAsciiWhitespace(ref) || HasAsciiWhitespace(hyp)) {
    return SplitWhitespace(hyp);
  }
  return SplitUtf8CharsNoWhitespace(hyp);
}

int EditDistance(const std::vector<std::string>& ref,
                 const std::vector<std::string>& hyp) {
  std::vector<int> prev(hyp.size() + 1);
  for (size_t j = 0; j <= hyp.size(); ++j) {
    prev[j] = static_cast<int>(j);
  }
  for (size_t i = 1; i <= ref.size(); ++i) {
    std::vector<int> cur(hyp.size() + 1);
    cur[0] = static_cast<int>(i);
    for (size_t j = 1; j <= hyp.size(); ++j) {
      const int sub = prev[j - 1] + (ref[i - 1] == hyp[j - 1] ? 0 : 1);
      const int ins = cur[j - 1] + 1;
      const int del = prev[j] + 1;
      cur[j] = std::min({sub, ins, del});
    }
    prev = std::move(cur);
  }
  return prev.back();
...
GreedyContext MakeGreedyContext(const std::filesystem::path& model_dir,
  ...
  context.acoustic_resource = std::make_shared<Zipformer2CtcOnnxResource>(
      model_path.string(), num_threads, context.tokens->BlankId());
  if (context.acoustic_resource->Info().vocab_size !=
...
DecodeOutput DecodeOneGreedy(GreedyContext* context,
  ...
  try {
    ...
    Zipformer2CtcOnnxBackend backend(context->acoustic_resource);
    const auto log_probs = RunGreedyStreaming(&backend, features);
