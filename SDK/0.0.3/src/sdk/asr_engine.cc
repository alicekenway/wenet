...
StatusOr<std::unique_ptr<AsrEngine>> CreateFlashlightEngine(
  ...
  try {
    ...
    auto acoustic_resource =
        std::make_shared<internal::sherpa_onnx_wenet::Zipformer2CtcOnnxResource>(
            package.sherpa_ctc_onnx.string(), config.num_threads,
            decoder_resource->BlankId());
    if (acoustic_resource->Info().vocab_size !=
    ...
    shared->acoustic_resource = std::move(acoustic_resource);
