...
class FlashlightDecoderResource {
 public:
  ...
  // and the base trie are shared. Only the small overlay trie, dynamic contact
  ...
  const std::shared_ptr<const RuntimeTrieLookahead>& RuntimeLookahead() const {
    return runtime_lookahead_;
  }
