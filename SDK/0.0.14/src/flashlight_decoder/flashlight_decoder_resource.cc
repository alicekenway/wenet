...
struct SharedDecoderResources {
  SharedDecoderResources(const std::filesystem::path& tokens_path,
      ...
      : am_tokens(tokens_path),
        output_words(words_path),
        token_fl_dict(MakeTokenDictionary(am_tokens)),
        word_fl_dict(MakeWordDictionary(output_words)) {}

  sherpa_onnx_wenet::TokenTable am_tokens;
  WordDictionary output_words;
  fl::lib::text::Dictionary token_fl_dict;
  fl::lib::text::Dictionary word_fl_dict;
...
FlashlightDecoderResource::FlashlightDecoderResource(
  ...
  auto max_fusion_lm = std::make_shared<MaxFusionLm>(
      shared->fixed_lms, shared->word_fl_dict, nullptr,
...
FlashlightDecoderResource::FlashlightDecoderResource(
    ...
    : shared_(base.shared_),
      ...
      runtime_lookahead_(base.shared_->GetRuntimeLookahead(base.word_lm_)),
...
FlashlightDecoderResource::WordFlDictionary() const {
  return shared_->word_fl_dict;
}
...
FlashlightDecoderResource::WordSpellings() const {
  return shared_->word_spellings;
}
