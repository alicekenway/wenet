...
Status FlashlightCtcStreamDecoder::Start() {
  ...
  try {
    if (impl_->resource->OverlayTrie()) {
      impl_->decoder = std::make_unique<MultiTrieLexiconDecoder>(
          MakeDecoderOptions(impl_->resource->Options()),
          impl_->resource->LexiconTrie(), impl_->resource->OverlayTrie(),
          impl_->resource->RuntimeLookahead(), impl_->resource->WordLm(),
          impl_->resource->SilenceId(),
          impl_->resource->BlankId(), impl_->resource->UnknownWordId(),
          std::vector<float>{}, false);
    } else {
