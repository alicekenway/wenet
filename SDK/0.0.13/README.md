# WeNet SDK 0.0.13: Faster Startup and Shared Decoder Resources

SDK 0.0.13 retains the flexible multi-LM decoder and optional inverse text
normalization.

Startup work in this version removes the temporary ONNX Runtime probe session,
loads token, word, lexicon, mapping, and KenLM resources only once while
constructing the decoder, and shares the loaded fixed KenLM resources with
runtime contact decode contexts. Each contact context still owns its dynamic
contact lexicon and combined trie.
