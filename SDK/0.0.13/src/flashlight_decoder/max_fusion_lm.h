...
struct RecentRawLmEvent {
  int word_id = -1;
  std::vector<fl::lib::text::LMStatePtr> model_states_before;
  float contribution = 0.0f;
};

struct MaxFusionLmState : fl::lib::text::LMState {
  std::vector<fl::lib::text::LMStatePtr> model_states;
  std::vector<RecentRawLmEvent> recent_raw_events;
  size_t raw_word_count = 0;
...
// Immutable fixed language models shared by the base decoder and every
// runtime decode context compiled from it.
struct SharedFixedLmResources {
  std::vector<LoadedFixedLm> models;
};
...
class MaxFusionLm final : public fl::lib::text::LM {
 public:
  MaxFusionLm(std::vector<FixedLmConfig> configs,
              ...
              std::shared_ptr<const OutputSequenceMapper> pre_lm_mapper =
                  nullptr);
  MaxFusionLm(std::shared_ptr<const SharedFixedLmResources> fixed_lms,
              std::shared_ptr<const DynamicContactLexicon> dynamic_slots,
              float word_score,
              std::shared_ptr<const OutputSequenceMapper> pre_lm_mapper =
                  nullptr);
  ...
  std::pair<fl::lib::text::LMStatePtr, float> ScoreForLexiconSmearing(
      const fl::lib::text::LMStatePtr& state, int user_token_id);

  const std::vector<LoadedFixedLm>& Models() const {
    return fixed_lms_->models;
  }
  const std::shared_ptr<const SharedFixedLmResources>& FixedResources() const {
    return fixed_lms_;
  }
 ...
 private:
  ...
  float ScoreOne(const std::vector<fl::lib::text::LMStatePtr>& model_states,
                 int user_token_id, MaxFusionLmState* out) const;

  std::shared_ptr<const SharedFixedLmResources> fixed_lms_;
