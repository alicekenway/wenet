...
FlashlightDecoderResource::FlashlightDecoderResource(
  ...
  for (const FixedLmConfig& lm : fixed_lms_) {
    for (const std::string& slot : lm.slots) {
      ...
      if (!output_words_.Contains(slot) || slot == unk_word ||
          am_tokens_.Contains(slot)) {
  ...
  auto max_fusion_lm = std::make_shared<MaxFusionLm>(
      ...
      pre_lm_mapper_);
  fixed_lm_resources_ = max_fusion_lm->FixedResources();
  for (const LoadedFixedLm& loaded : fixed_lm_resources_->models) {
...
FlashlightDecoderResource::FlashlightDecoderResource(
    ...
    : am_tokens_(base.am_tokens_),
      ...
      fixed_lm_resources_(base.fixed_lm_resources_),
  ...
  word_lm_ = std::make_shared<MaxFusionLm>(
      fixed_lm_resources_, dynamic_contacts_, options_.word_score,
      pre_lm_mapper_);
  options_.lm_weight = 1.0;
}
