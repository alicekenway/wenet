...
OUT_DIR="${OUT_DIR:-${ROOT}/test/0.0.16/package}"
...
COMPACT_LEXICON_TOOL="${COMPACT_LEXICON_TOOL:-${ROOT}/wenet/SDK/0.0.16/build/compact_lexicon_tool}"
LEXICON_REPORT="${LEXICON_REPORT:-${OUT_DIR}.lexicon_report.json}"
MAPPING="${MAPPING:-}"
...
for file in "${AM_MODEL}" "${TOKENS_FILE}" "${LM_SEARCH_JSON}" \
            "${WORDS_FILE}" "${LEXICON_FILE}" "${COMPACT_LEXICON_TOOL}"; do
  require_file "${file}"
done
itn_fields=0
if [[ -n "${SENTENCEPIECE_MODEL}" ]]; then
  require_file "${SENTENCEPIECE_MODEL}"
fi
...
if [[ "${SMEARING,,}" != max ]]; then
  echo "0.0.16 compact multi-slot packages require SMEARING=max" >&2
  exit 1
fi
...
if [[ -n "${SENTENCEPIECE_MODEL}" ]]; then
  copy_runtime_file "${SENTENCEPIECE_MODEL}" "${OUT_DIR}/sentencepiece.model"
fi
...
if "${SENTENCEPIECE_MODEL}":
  manifest["sentencepiece_model"]="sentencepiece.model"
if "${ITN_LANGUAGE}":
...
# Compile from the complete staged 0.0.14/0.0.15-compatible source package.
# lexicon.txt and the compiler report are intentionally excluded from the
# production package after verification.
"${COMPACT_LEXICON_TOOL}" compile \
  --source-package "${OUT_DIR}" \
  --output "${OUT_DIR}/lexicon.bin" \
  --report "${LEXICON_REPORT}"
"${COMPACT_LEXICON_TOOL}" verify \
  --source-package "${OUT_DIR}" \
  --input "${OUT_DIR}/lexicon.bin"

python3 - "${OUT_DIR}/sdk_model.json" <<'PY'
import json
import pathlib
import sys
...
manifest["decoder_type"] = "flashlight_compact_lexicon_kenlm"
manifest["lexicon"] = "lexicon.bin"
manifest["lexicon_format"] = "compact_trie_v1"
path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n",
                encoding="utf-8")
PY
rm -f -- "${OUT_DIR}/lexicon.txt"

checksum_files=("sdk_model.json" "model.onnx" "tokens.txt" "words.txt"
  "lexicon.bin" "lm_search.json" "output_mapping.txt" "${lm_outputs[@]}")
if [[ -n "${SENTENCEPIECE_MODEL}" ]]; then
  checksum_files+=("sentencepiece.model")
fi
if [[ -n "${FINAL_MAPPING}" ]]; then
...
echo "prepared 0.0.16 compact multi-LM package: ${OUT_DIR}"
