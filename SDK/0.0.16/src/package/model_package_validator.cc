...
Status ValidateFlashlightPackage(const ModelPackage& package) {
  if (!package.has_compact_lexicon ||
      package.lexicon_format != "compact_trie_v1") {
    return Status::InvalidArgument(
        "SDK 0.0.16 Flashlight runtime requires compact_trie_v1 lexicon.bin");
  }
  ...
  for (const auto& item :
       {std::pair{package.sherpa_ctc_onnx, "model.onnx"},
        ...
        std::pair{package.lexicon_bin, "lexicon.bin"},
  ...
  try {
    ...
    const auto lexicon = flashlight_decoder::CompactLexicon::Load(
        package.lexicon_bin,
        flashlight_decoder::ComputeCompactLexiconDependencyHash(package));
    if (lexicon->TokenCount() !=
            static_cast<uint32_t>(tokens.ModelVocabSize()) ||
        lexicon->WordCount() != static_cast<uint32_t>(words.Size()) ||
        lexicon->BlankId() != tokens.Id(package.blank_token) ||
        lexicon->SilenceId() != tokens.Id(package.sil_token) ||
        lexicon->UnknownWordId() != words.Id(package.unk_word)) {
      return Status::InvalidArgument(
          "compact lexicon metadata does not match package dictionaries");
    }
    ...
    for (const std::string& slot : slots) {
      if (!lexicon->WordSpellings(words.Id(slot)).empty()) {
        return Status::InvalidArgument(
...
Status ValidateFlashlightPackageLayout(const ModelPackage& package) {
  if (!package.has_compact_lexicon ||
      package.lexicon_format != "compact_trie_v1") {
    return Status::InvalidArgument(
        "SDK 0.0.16 Flashlight runtime requires compact_trie_v1 lexicon.bin");
  ...
  for (const FixedLmConfig& lm : package.fixed_lms) {
    Status status = RequireFile(lm.path, lm.filename.c_str());
    if (!status.ok()) return status;
    has_slots = has_slots || !lm.slots.empty();
  }
  if (has_slots && LowerAscii(package.flashlight_options.smearing) != "max") {
    return Status::InvalidArgument("slot-ready package requires smearing=max");
  }
  return Status::Ok();
}
...
Status ValidateModelPackageLayout(const ModelPackage& package) {
  ...
  if (package.has_itn) {
    Status status = RequireFile(package.itn_tagger_fst, "English ITN tagger FST");
    if (!status.ok()) return status;
    status = RequireFile(package.itn_verbalizer_fst,
                         "English ITN verbalizer FST");
    if (!status.ok()) return status;
  }
  if (!package.has_flashlight_decoder) {
    return ValidateModelPackage(package);
  }
  if (package.sample_rate != 16000) {
    return Status::InvalidArgument(
        "only 16 kHz audio is supported by this Flashlight CTC package");
  }
  return ValidateFlashlightPackageLayout(package);
}
...
ModelPackageReport InspectModelPackage(const ModelPackage& package) {
  ...
  report.lines.push_back(std::string("ITN: ") +
                         (package.has_itn ? "enabled (en)" : "disabled"));
  if (package.has_itn) {
    AddFileLine("ITN tagger", package.itn_tagger_fst, &report);
    AddFileLine("ITN verbalizer", package.itn_verbalizer_fst, &report);
  }
  if (package.has_flashlight_decoder) {
    AddFileLine("model.onnx", package.sherpa_ctc_onnx, &report);
    AddFileLine("tokens.txt", package.tokens_txt, &report);
    if (!package.sentencepiece_model.empty()) {
      AddFileLine("sentencepiece_model", package.sentencepiece_model, &report);
    } else {
      report.lines.push_back("sentencepiece_model: absent (automatic DCC unavailable; explicit AM paths supported)");
    }
    AddFileLine("words.txt", package.words_txt, &report);
    AddFileLine("lexicon.bin", package.lexicon_bin, &report);
    ...
    try {
      ...
      report.lines.push_back("lexicon entry count: " +
                             std::to_string(lexicon->LexiconEntryCount()));
      report.lines.push_back("compact lexicon nodes/edges: " +
                             std::to_string(lexicon->NodeCount()) + "/" +
                             std::to_string(lexicon->EdgeCount()));
      report.lines.push_back("compact lexicon bytes: " +
                             std::to_string(lexicon->FileSize()));
