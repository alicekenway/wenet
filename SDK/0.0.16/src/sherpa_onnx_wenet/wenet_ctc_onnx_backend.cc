...
WenetCtcOnnxResource::WenetCtcOnnxResource(
    const std::string& model_path, int num_threads, int blank_id)
    : WenetCtcOnnxResource(
          std::make_shared<CtcOnnxSession>(model_path, num_threads),
          blank_id) {}

WenetCtcOnnxResource::WenetCtcOnnxResource(
    std::shared_ptr<CtcOnnxSession> session, int blank_id)
    : session_(std::move(session)) {
  if (!session_) throw std::invalid_argument("ONNX session is null");
...
void WenetCtcOnnxResource::ReadMetadata(int blank_id) {
  Ort::AllocatorWithDefaultOptions allocator;
  Ort::ModelMetadata metadata = session_->Session().GetModelMetadata();
...
std::vector<std::string> WenetCtcOnnxResource::GetNodeNames(bool input) const {
  Ort::AllocatorWithDefaultOptions allocator;
  const size_t count = input ? session_->Session().GetInputCount()
                             : session_->Session().GetOutputCount();
  std::vector<std::string> names;
  names.reserve(count);
  for (size_t i = 0; i < count; ++i) {
    Ort::AllocatedStringPtr ptr =
        input ? session_->Session().GetInputNameAllocated(i, allocator)
              : session_->Session().GetOutputNameAllocated(i, allocator);
...
void WenetCtcOnnxResource::ReadNodes() {
  ...
  if (TensorType(session_->Session().GetInputTypeInfo(feature_index)) !=
      ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT) {
    throw std::runtime_error("wenet_ctc feature input must be float");
  }
  const std::vector<int64_t> feature_shape =
      TensorShape(session_->Session().GetInputTypeInfo(feature_index));
  ...
  if (TensorType(session_->Session().GetOutputTypeInfo(log_index)) !=
      ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT) {
    throw std::runtime_error("wenet_ctc log_probs output must be float");
  }
  const std::vector<int64_t> log_shape =
      TensorShape(session_->Session().GetOutputTypeInfo(log_index));
  if (log_shape.size() != 3 ||
...
void WenetCtcOnnxBackend::Forward(
  ...
  std::vector<Ort::Value> outputs = resource_->session_->Session().Run(
