...
using SlotSpokenForm = ContactSpokenForm;

struct SlotValueEntry {
  std::string value_id;
  std::string display_name;
  std::vector<SlotSpokenForm> spoken_forms;
};

struct SlotClass {
  std::string slot_token;
  std::vector<SlotValueEntry> values;
};
...
struct ContactListOptions {
  ...
  int max_total_dynamic_forms = 100000;
  // Automatic spellings combine up to three paths PER WORD. These limits
  // reject oversized contexts; they never select a truncated set of paths.
  int max_token_segmentations_per_form = 100000;
...
struct DecodeContextConfig {
  ...
  // Compatibility input. Entries are compiled as values of <CONTACT>.
