...
StatusOr<ModelPackage> LoadModelPackage(const EngineConfig& config) {
  ...
  if (FileExists(package.manifest)) {
    ...
    } else if (FileExists(package.sherpa_ctc_onnx) &&
               FileExists(package.tokens_txt)) {
      package.decoder_type = "ctc_greedy";
    }
  }
  ...
  package.has_flashlight_decoder =
      package.decoder_type == "flashlight_lexicon_kenlm";
  package.has_ctc_greedy = package.decoder_type == "ctc_greedy" ||
                           package.decoder_type == "onnx_ctc_greedy";
