...
Status ValidateCtcGeometry(const StreamingCtcBackend& backend) {
...
Status FlashlightAsrStream::EnsureInitialized() {
  ...
  try {
    ...
    impl_->AddDebugLog("stream_init android_reduced");
    return Status::Ok();
...
Status FlashlightAsrStream::AcceptPcm16(const int16_t* samples,
  ...
  if (num_samples > 0) {
    impl_->samples.insert(impl_->samples.end(), samples,
                          samples + num_samples);
  }
...
bool FlashlightAsrStream::DecodeReady() const {
  return input_finished_ && !final_emitted_;
}
...
Status FlashlightAsrStream::ProcessQueuedFeatures() {
  ...
  try {
    ...
    sherpa_onnx_wenet::WhisperFeatureExtractor(feature_options)
        .ExtractPcm16(impl_->samples.data(), impl_->samples.size(), &features);

    Status status = ValidateCtcGeometry(*impl_->backend);
    if (!status.ok()) {
      return impl_->RecordStatus(status);
    }
    const auto& info = impl_->backend->Info();
    impl_->AddDebugLog("extract_features frames=" +
    ...
    while (start + info.input_window_frames <=
      ...
      impl_->backend->Forward(chunk.data(), info.input_window_frames,
                              &log_probs);
      std::vector<float> flat;
      FlattenFrames(log_probs, &flat);
      status = impl_->decoder->DecodeChunk(
    ...
    if (!forwarded || start < static_cast<int>(features.size())) {
      ...
      impl_->AddDebugLog("decode_final_chunk frames=" +
                         std::to_string(log_probs.size()) + " vocab=" +
                         std::to_string(info.vocab_size));
    }
    ...
    return ExceptionStatus("Flashlight Android reduced decode failed", e);
  }
}
...
Status FlashlightAsrStream::ForwardCurrentWindow(bool final_padding) {
  ...
  return Status::Unavailable(
      "ForwardCurrentWindow is not used by Android reduced Flashlight stream");
}

Status FlashlightAsrStream::EmitPartialIfAvailable() { return Status::Ok(); }
...
Status FlashlightAsrStream::FinalizeIfReady() {
  ...
  Status status = ProcessQueuedFeatures();
  if (!status.ok()) {
    return impl_->RecordStatus(status);
  }
  try {
...
Status FlashlightAsrStream::ForwardCurrentWindow(bool final_padding) {
  ...
  try {
    ...
    if (final_padding) {
      ...
      const int consumed_new = impl_->decoded_chunks == 0 ? window : shift;
      impl_->pending_new_frames =
