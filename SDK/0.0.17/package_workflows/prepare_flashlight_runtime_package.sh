...
OUT_DIR="${OUT_DIR:-${ROOT}/test/0.0.17/package}"
...
COMPACT_LEXICON_TOOL="${COMPACT_LEXICON_TOOL:-${ROOT}/wenet/SDK/0.0.17/build/compact_lexicon_tool}"
...
if [[ "${SMEARING,,}" != max ]]; then
  echo "0.0.17 compact multi-slot packages require SMEARING=max" >&2
  exit 1
fi
...
echo "prepared 0.0.17 compact multi-LM package: ${OUT_DIR}"
