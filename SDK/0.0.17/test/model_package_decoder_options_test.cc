...
int main() {
  ...
  bool ok = true;
  using asr_sdk::internal::FindJsonStringValue;
  ok &= Expect(FindJsonStringValue(R"({"s":"\u2581"})", "s", "") == "▁", "Unicode escapes preserve token names");
  ok &= Expect(FindJsonStringValue(R"({"s":"\ud83d\ude00"})", "s", "") == "😀", "surrogate pair decoded");
  ok &= Expect(!asr_sdk::internal::ParseJson(R"({"s":"\ud800"})").ok(), "unpaired surrogate rejected");
  ok &= Expect(p.sentencepiece_model == dir / "sentencepiece.model",
               "optional tokenizer path resolved relative to package");
  ...
  ok &= Expect(p.has_flashlight_decoder && !p.has_compact_lexicon,
               "0.0.14/0.0.15 text package remains a compiler input");
  ...
  ok &= Expect(itn_package.ok() && itn_package.value().sentencepiece_model.empty(),
               "older packages do not require a tokenizer at load time");
  ...
  Write(dir / "sdk_model.json", R"({
    ...
    "model_path":"am.onnx", "tokens":"tokens.txt", "words":"words.txt",
    "lexicon":"lexicon.bin", "lexicon_format":"compact_trie_v1",
    "lm_search":"lm_search.json"
  })");
  auto compact_package = asr_sdk::internal::LoadModelPackage(config);
  ok &= Expect(compact_package.ok() &&
                   compact_package.value().has_flashlight_decoder &&
                   compact_package.value().has_compact_lexicon &&
                   compact_package.value().lexicon_bin == dir / "lexicon.bin",
               "0.0.17 compact package fields parsed");
  ok &= Expect(!compact_package.value().enable_weight_prepacking &&
                   !compact_package.value().use_device_allocator_for_initializers,
               "missing memory options use new defaults");
  for (const std::string option : {"enable_weight_prepacking", "use_device_allocator_for_initializers"}) {
    for (const std::string value : {"true", "false", "1", "null", "\"false\""}) {
      Write(dir / "sdk_model.json", "{\"" + option + "\":" + value + "}");
      auto result = asr_sdk::internal::LoadModelPackage(config);
      bool valid = value == "true" || value == "false";
      ok &= Expect(result.ok() == valid, "strict memory option: " + option + "=" + value);
      if (result.ok()) ok &= Expect((option == "enable_weight_prepacking" ?
          result.value().enable_weight_prepacking : result.value().use_device_allocator_for_initializers) == (value == "true"), "memory option value");
    }
  }
  fs::remove_all(dir);
  return ok ? 0 : 1;
