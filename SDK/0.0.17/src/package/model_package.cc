...
StatusOr<std::vector<FixedLmConfig>> LoadFixedLms(
  ...
  for (const auto& item : parsed.value().AsObject()) {
...
StatusOr<ModelPackage> LoadModelPackage(const EngineConfig& config) {
  ...
  if (FileExists(package.manifest)) {
    ...
    auto parsed_options = ParseJson(manifest_json);
    if (!parsed_options.ok()) return parsed_options.status();
    if (!parsed_options.value().IsObject()) return Status::InvalidArgument("sdk_model.json must be an object");
    const auto& fields = parsed_options.value().AsObject();
    for (const auto& option : std::vector<std::pair<std::string, bool*>>{
        {"enable_weight_prepacking", &package.enable_weight_prepacking},
        {"use_device_allocator_for_initializers", &package.use_device_allocator_for_initializers}}) {
      auto found = fields.find(option.first);
      if (found != fields.end()) {
        if (!found->second.IsBool()) return Status::InvalidArgument(option.first + " must be boolean");
        *option.second = found->second.AsBool();
      }
    }
    ...
    package.lexicon_bin = package.lexicon_txt;
    const std::string sentencepiece =
        FindJsonStringValue(manifest_json, "sentencepiece_model", "");
    if (!sentencepiece.empty()) {
      package.sentencepiece_model = ResolveUnder(package.root, sentencepiece);
    }
    package.lexicon_format =
        FindJsonStringValue(manifest_json, "lexicon_format", "");
    ...
    package.itn_language = FindJsonStringValue(manifest_json, "itn_language", "");
    const std::string itn_tagger =
        FindJsonStringValue(manifest_json, "itn_tagger", "");
    const std::string itn_verbalizer =
        FindJsonStringValue(manifest_json, "itn_verbalizer", "");
    const int itn_fields = static_cast<int>(!package.itn_language.empty()) +
                           static_cast<int>(!itn_tagger.empty()) +
                           static_cast<int>(!itn_verbalizer.empty());
    if (itn_fields != 0 && itn_fields != 3) {
      return Status::InvalidArgument(
          "itn_language, itn_tagger, and itn_verbalizer must be set together");
    }
    package.has_itn = itn_fields == 3;
    if (package.has_itn) {
      if (package.itn_language != "en") {
        return Status::InvalidArgument("SDK 0.0.17 supports only itn_language=en");
      }
      package.itn_tagger_fst = ResolveUnder(package.root, itn_tagger);
      package.itn_verbalizer_fst = ResolveUnder(package.root, itn_verbalizer);
    }
    ...
    package.lexicon_bin = package.root / "lexicon.bin";
  ...
  package.has_compact_lexicon =
      package.decoder_type == "flashlight_compact_lexicon_kenlm";
  package.has_flashlight_decoder = package.has_compact_lexicon ||
      package.decoder_type == "flashlight_lexicon_kenlm";
