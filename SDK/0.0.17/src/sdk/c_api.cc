...
int asr_sdk_context_add_slot_form(
    AsrSdkContext* context, const char* slot_token, const char* value_id,
    const char* display_name, const char* spoken_form,
    const char* const* am_tokens, int num_am_tokens, int logical_word_count) {
  const int mutable_status = ContextMutationStatus(context);
  if (mutable_status != ASR_SDK_STATUS_OK) return mutable_status;
  if (slot_token == nullptr || value_id == nullptr || display_name == nullptr ||
      spoken_form == nullptr) {
    return SetContextError(context, asr_sdk::Status::InvalidArgument(
        "slot_token, value_id, display_name, and spoken_form are required"));
  }
  if (num_am_tokens < 0 || logical_word_count < 0 ||
      (num_am_tokens > 0 && am_tokens == nullptr)) {
    return SetContextError(context, asr_sdk::Status::InvalidArgument(
        "invalid AM-token or logical-word count"));
  }
  const std::string slot(slot_token);
  const std::string id(value_id);
  const std::string display(display_name);
  if (slot.size() < 3 || slot.front() != '<' || slot.back() != '>' ||
      id.empty() || display.empty() || std::string(spoken_form).empty()) {
    return SetContextError(context, asr_sdk::Status::InvalidArgument(
        "slot token and slot value strings must be non-empty and valid"));
  }
  asr_sdk::SlotSpokenForm form;
  form.text = spoken_form;
  form.logical_word_count = logical_word_count;
  for (int i = 0; i < num_am_tokens; ++i) {
    if (am_tokens[i] == nullptr) {
      return SetContextError(context,
          asr_sdk::Status::InvalidArgument("am_tokens contains null"));
    }
    form.am_tokens.emplace_back(am_tokens[i]);
  }
  auto slot_it = std::find_if(context->config.slots.begin(),
                              context->config.slots.end(),
                              [&](const asr_sdk::SlotClass& item) {
                                return item.slot_token == slot;
                              });
  if (slot_it == context->config.slots.end()) {
    context->config.slots.push_back(asr_sdk::SlotClass{});
    slot_it = std::prev(context->config.slots.end());
    slot_it->slot_token = slot;
  }
  auto value_it = std::find_if(slot_it->values.begin(), slot_it->values.end(),
                               [&](const asr_sdk::SlotValueEntry& item) {
                                 return item.value_id == id;
                               });
  if (value_it == slot_it->values.end()) {
    slot_it->values.push_back(asr_sdk::SlotValueEntry{});
    value_it = std::prev(slot_it->values.end());
    value_it->value_id = id;
    value_it->display_name = display;
  } else if (value_it->display_name != display) {
    return SetContextError(context, asr_sdk::Status::InvalidArgument(
        "repeated slot value has a conflicting display_name"));
  }
  value_it->spoken_forms.push_back(std::move(form));
  context->last_status = asr_sdk::Status::Ok();
  SetGlobalStatus(asr_sdk::Status::Ok());
  return ASR_SDK_STATUS_OK;
}
