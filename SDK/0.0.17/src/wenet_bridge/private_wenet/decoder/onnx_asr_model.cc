...
void OnnxAsrModel::Read(const std::string& model_dir) {
  Ort::SessionOptions memory_options;
  memory_options.SetIntraOpNumThreads(threads_);
  memory_options.AddConfigEntry("session.disable_prepacking", prepacking_ ? "0" : "1");
  memory_options.AddConfigEntry("session.use_device_allocator_for_initializers", device_initializers_ ? "1" : "0");
  ...
  try {
    ...
    encoder_session_ = std::make_shared<Ort::Session>(
        env_, ToWString(encoder_onnx_path).c_str(), memory_options);
    rescore_session_ = std::make_shared<Ort::Session>(
        env_, ToWString(rescore_onnx_path).c_str(), memory_options);
    ctc_session_ = std::make_shared<Ort::Session>(
        env_, ToWString(ctc_onnx_path).c_str(), memory_options);
#else
    encoder_session_ = std::make_shared<Ort::Session>(
        env_, encoder_onnx_path.c_str(), memory_options);
    rescore_session_ = std::make_shared<Ort::Session>(
        env_, rescore_onnx_path.c_str(), memory_options);
