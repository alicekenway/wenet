...
std::unique_ptr<StreamingCtcBackend> CreateStreamingCtcBackend(
    ...
    OrtLogLevel log_level, bool prepacking, bool device_initializers) {
  auto session =
      std::make_shared<CtcOnnxSession>(model_path, num_threads, log_level, prepacking, device_initializers);
