...
void Zipformer2CtcOnnxResource::ReadNodes() {
  ...
  const TensorSpec feature_spec =
      ReadTensorSpec(input_names_[0], session_->Session().GetInputTypeInfo(0));
  ...
  const TensorSpec log_prob_spec =
      ReadTensorSpec(output_names_[0], session_->Session().GetOutputTypeInfo(0));
  ...
  for (size_t i = 1; i < input_names_.size(); ++i) {
    state_input_specs_.push_back(
        ReadTensorSpec(input_names_[i], session_->Session().GetInputTypeInfo(i)));
    state_output_specs_.push_back(
        ReadTensorSpec(output_names_[i], session_->Session().GetOutputTypeInfo(i)));
