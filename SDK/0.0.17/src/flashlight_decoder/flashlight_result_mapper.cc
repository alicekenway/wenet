...
std::vector<DecodedWord> MapContactAwareWords(
    ...
    const OutputSequenceMapper& mapper) {
  std::vector<DecodedWord> output;
  size_t begin = 0;
  while (begin < raw.size()) {
    ...
    std::vector<DecodedWord> mapped = mapper.RewriteWords(span, begin);
...
DecodedHypothesis ConvertFlashlightResult(
  ...
  if (!resource.HasSlotContext()) {
    ...
    hyp.am_mapped_words =
        resource.PreLmMapper().RewriteWords(hyp.raw_words);
    hyp.mapped_words =
        resource.FinalMapper().RewriteWords(hyp.am_mapped_words);
    return hyp;
  }
  ...
  hyp.am_mapped_words =
      MapContactAwareWords(hyp.raw_words, resource.PreLmMapper());
  hyp.mapped_words =
      MapContactAwareWords(hyp.am_mapped_words, resource.FinalMapper());
  return hyp;
}
