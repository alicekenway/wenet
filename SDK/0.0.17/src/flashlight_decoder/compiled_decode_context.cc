...
std::string MakeFingerprint(const FlashlightDecoderResource& base,
  ...
  for (const FixedLmConfig& lm : base.FixedLms()) {
    HashBytes(&hash, lm.filename);
    HashBytes(&hash, std::to_string(lm.weight));
  }
  for (const RuntimeContactForm& form : contacts.Forms()) {
    HashBytes(&hash, form.spoken_form);
    HashBytes(&hash, form.slot_token);
    ...
    for (const RuntimeContactCandidate& candidate : form.candidates) {
      HashBytes(&hash, candidate.value_id);
...
CompiledDecodeContext::Compile(FlashlightDecoderResourcePtr base_resource,
  ...
  if (!base_resource->SupportsRuntimeSlots()) {
    return Status::FailedPrecondition(
        "runtime slots require at least one configured bias LM");
  }
  ...
  try {
    ...
    auto overlay_trie =
        out->dynamic_contacts_->BuildOverlayTrie(*base_resource);
    out->slot_decoder_resource_ =
        FlashlightDecoderResource::CreateSlotContextResource(
            *base_resource, std::move(overlay_trie),
            out->dynamic_contacts_);
    out->fingerprint_ = MakeFingerprint(*base_resource, *out->dynamic_contacts_);
    out->empty_ = false;
    return std::shared_ptr<const CompiledDecodeContext>(std::move(out));
  } catch (const std::exception& e) {
    return Status::Internal(std::string("failed to compile runtime slots: ") +
                            e.what());
  }
}
