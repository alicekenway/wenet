...
Processor::Processor(const std::string& tagger_path,
  ...
  if (!tagger_ || !verbalizer_) {
    throw std::runtime_error("failed to read WeTextProcessing FST files");
  }
...
std::string Processor::ShortestPath(const StdVectorFst& lattice) const {
  StdVectorFst shortest_path;
  fst::ShortestPath(lattice, &shortest_path, 1, true);

  std::string output;
  StringPrinter<StdArc> printer(fst::StringTokenType::UTF8);
  printer(shortest_path, &output);
  return output;
}

std::string Processor::Compose(const std::string& input,
                               const StdVectorFst* fst) const {
  StdVectorFst input_fst;
  StringCompiler<StdArc> compiler(fst::StringTokenType::UTF8);
...
std::string Processor::Tag(const std::string& input) const {
  if (input.empty()) {
    return "";
  }
...
std::string Processor::Verbalize(const std::string& input) const {
  if (input.empty()) {
    return "";
  }
...
std::string Processor::Normalize(const std::string& input) const {
  return Verbalize(Tag(input));
}

std::string Processor::Normalize(const std::string& input) {
  return static_cast<const Processor&>(*this).Normalize(input);
}
