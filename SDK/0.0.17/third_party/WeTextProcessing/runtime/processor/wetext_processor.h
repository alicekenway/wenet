...
class Processor {
 public:
  ...
  std::string Tag(const std::string& input) const;
  std::string Verbalize(const std::string& input) const;
  std::string Normalize(const std::string& input) const;
  // ABI-compatible entry point used by the WeNet post_processor archive.
  std::string Normalize(const std::string& input);

 private:
  std::string ShortestPath(const StdVectorFst& lattice) const;
  std::string Compose(const std::string& input, const StdVectorFst* fst) const;
