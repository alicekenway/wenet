...
Trie::insert(const std::vector<int>& indices, int label, float score) {
  ...
  // kTrieInitialLabelReserve is not a cap.  Dropping labels here loses valid
  // homophones and runtime contact candidates before the decoder can score
  // their language-model state.
...
void smearNode(TrieNodePtr node, SmearingMode smearMode) {
  ...
  for (auto score : node->scores) {
    ...
    } else if (smearMode == SmearingMode::RUNTIME_CONTACT_MAX &&
               score > node->maxScore) {
      node->maxScore = score;
    }
  }
  for (auto child : node->children) {
    ...
    if (smearMode == SmearingMode::LOGADD) {
      node->maxScore = TrieLogAdd(node->maxScore, childNode->maxScore);
        ...
        (smearMode == SmearingMode::MAX ||
         smearMode == SmearingMode::RUNTIME_CONTACT_MAX) &&
