...
std::pair<LMStatePtr, float> KenLM::scoreUnknown(const LMStatePtr& state,
                                                int cacheKey) {
  auto inState = std::static_pointer_cast<KenLMState>(state);
  auto outState = inState->child<KenLMState>(cacheKey);
  const float score = model_->BaseScore(
      inState->ken(), vocab_->NotFound(), outState->ken());
  return std::make_pair(std::move(outState), score);
}
