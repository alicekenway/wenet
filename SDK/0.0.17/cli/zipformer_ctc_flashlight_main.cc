...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (wav.sample_rate != 16000 || wav.num_channels != 1) {
      std::cerr << "Zipformer CTC Flashlight tool expects 16 kHz mono WAV\n";
      return 1;
    }
    ...
    asr_sdk::internal::FixedLmConfig lm_config;
    lm_config.filename = std::filesystem::path(args.lm).filename().string();
    lm_config.path = args.lm;
    lm_config.type = asr_sdk::internal::FixedLmType::kNgram;
    lm_config.weight = args.decoder.lm_weight;
    lm_config.clip = false;
    auto decoder_resource = std::make_shared<FlashlightDecoderResource>(
        args.tokens, args.words, args.lexicon,
        std::vector<asr_sdk::internal::FixedLmConfig>{lm_config}, args.mapping,
        std::filesystem::path(), args.decoder, args.blank_token,
        args.sil_token, args.unk_word,
        std::max(0.0, -args.decoder.word_score));
    ...
    std::cout << "pre-LM mapping rules: "
              << decoder_resource->PreLmMapper().RuleCount() << "\n";
