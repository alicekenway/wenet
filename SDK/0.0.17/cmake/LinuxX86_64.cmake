...
if(ASR_SDK_ENABLE_FLASHLIGHT_DECODER)
  target_link_libraries(asr_sdk_ctc_core PUBLIC asr_sdk::flashlight_text asr_sdk::sentencepiece)
endif()
...
target_include_directories(asr_sdk BEFORE PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/src/wenet_bridge/private_wenet)
...
if(ASR_SDK_BUILD_TOOLS)
  ...
  if(ASR_SDK_ENABLE_FLASHLIGHT_DECODER)
    # Host packaging tool. It is intentionally not installed and is never
    # part of the Android build or the production SDK shared library.
    add_executable(compact_lexicon_tool
      ../../kenlm_lm/tools/compact_lexicon/main.cc
      src/flashlight_decoder/compact_lexicon_builder.cc
      src/package/model_package.cc
      src/sdk/status.cc
      src/utils/file_utils.cc
      src/utils/json.cc
    )
    target_include_directories(compact_lexicon_tool PRIVATE
      ${CMAKE_CURRENT_SOURCE_DIR}/include
      ${CMAKE_CURRENT_SOURCE_DIR}/src
      ${ASR_SDK_WENET_INCLUDE_DIRS}
    )
    target_link_libraries(compact_lexicon_tool PRIVATE
      asr_sdk::ctc_core
      dl
      pthread
    )
...
if(ASR_SDK_BUILD_TESTS AND ASR_SDK_ENABLE_FLASHLIGHT_DECODER)
  enable_testing()
  add_executable(word_spelling_test test/word_spelling_test.cc src/sdk/status.cc)
  target_link_libraries(word_spelling_test PRIVATE asr_sdk::ctc_core)
  add_test(NAME word_spelling_test COMMAND word_spelling_test)
  add_executable(dcc_automatic_test test/dcc_automatic_test.cc
    src/package/model_package.cc src/utils/file_utils.cc src/utils/json.cc)
  target_link_libraries(dcc_automatic_test PRIVATE asr_sdk asr_sdk::ctc_core)
  add_executable(compact_lexicon_test
    test/compact_lexicon_test.cc
    src/flashlight_decoder/compact_lexicon_builder.cc
  )
  target_include_directories(compact_lexicon_test PRIVATE
    ${CMAKE_CURRENT_SOURCE_DIR}/src
    ${CMAKE_CURRENT_SOURCE_DIR}/include
  )
  target_link_libraries(compact_lexicon_test PRIVATE asr_sdk::ctc_core)
  add_test(NAME compact_lexicon_test COMMAND compact_lexicon_test)
  ...
  if(ASR_SDK_BASH_EXECUTABLE)
    ...
    add_test(NAME prepare_flashlight_runtime_package_test
             COMMAND ${CMAKE_COMMAND} -E env COMPACT_LEXICON_TOOL=$<TARGET_FILE:compact_lexicon_tool> ${ASR_SDK_BASH_EXECUTABLE}
