...
struct AsrResult {
  std::string text;
  // Original best-hypothesis text before optional inverse normalization.
  // Empty when ITN is disabled. N-best entries, tokens, scores, and entities
  // always retain their decoder-space values.
  std::string raw_text;
  bool is_final = false;
  float confidence = 0.0f;
  std::vector<TokenResult> tokens;
