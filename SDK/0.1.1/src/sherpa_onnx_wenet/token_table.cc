...
TokenTable::TokenTable(const std::filesystem::path& path) {
  ...
  while (std::getline(in, line)) {
    ...
    if (token_to_id_.find(token) != token_to_id_.end()) {
      throw std::runtime_error("duplicate token in tokens.txt: " + token);
    }
    if (!id_to_token_[static_cast<size_t>(id)].empty()) {
      throw std::runtime_error("duplicate token id in tokens.txt: " +
                               std::to_string(id));
    }
...
bool TokenTable::Contains(const std::string& token) const {
  return token_to_id_.find(token) != token_to_id_.end();
}

int TokenTable::Id(const std::string& token) const {
  const auto it = token_to_id_.find(token);
  if (it == token_to_id_.end()) {
    throw std::runtime_error("token not in tokens.txt: " + token);
  }
  return it->second;
}
