...
model's `model_type` metadata. 0.1.1 supports `zipformer2`, `wenet_ctc`, and
NeMo CTC exports such as `EncDecHybridRNNTCTCBPEModel`.
...
WeNet CTC models. Use `feature_type=nemo` for NeMo FastConformer CTC packages:
it keeps Kaldi-style fbank parameters but scales PCM to unit range before
feature extraction. The default `whisper` mode is kept for packages that were
