...
class ItnProcessor {
 ...
 private:
  // wetext::Processor owns mutable FST traversal scratch state. One processor
  // is shared by all streams from an engine, so normalization must be
  // serialized even though acoustic decoding remains concurrent.
