# Development Plan: Flashlight-Text + KenLM Shallow Fusion for WeNet SDK 0.0.3
...
### 0.0.3 independence requirement

Version `0.0.3` is developed from the `0.0.2` implementation and may reuse code patterns, build knowledge, and tested components from `0.0.2`.

However, the finished `0.0.3` SDK must run independently:

- It must build from the `0.0.3` source tree without reading files from the `0.0.2` SDK directory.
- It must load a self-contained `0.0.3` model package.
- It must not require `0.0.2` binaries, generated WFST graphs, temporary build outputs, scripts, or runtime resource paths.
- Any code copied or adapted from `0.0.2` must become part of the `0.0.3` source or package-generation tooling.
- The final validation must run from the `test/0.0.3` directory, not from `test/0.0.2`.
...
11. Deliver `0.0.3` as an independent SDK/package flow, not as a runtime wrapper around `0.0.2`.
...
For the initial validation data, `wenetspeech_lm_char.txt` may already be separated into character-like output units. That is acceptable if `words.txt`, `lexicon.txt`, and the KenLM training text use the same segmentation consistently. In this plan, "word" means the Flashlight lexicon output unit, which may be a full word, Chinese character, acronym, or other stable word-like unit.
...
### Initial local resources for 0.0.3 validation

Use these repository-local paths for the first full AM + LM validation:

```text
Acceptance test directory:
/home/jinyang_wang/Dev/ASR/ASR_wenet/test/0.0.3

Evaluation metadata:
/home/jinyang_wang/Dev/ASR/ASR_wenet/data/hf_wenetspeech_test_net/wenetspeech_test_net_sample_2000/metadata.jsonl
...
KenLM tooling/output directory:
/home/jinyang_wang/Dev/ASR/ASR_wenet/LM/kenlm_lm
```

Do not decode the full 2000-sample metadata file for the first acceptance run. Create a deterministic subset of about 100 utterances, store the sampled metadata under `test/0.0.3`, and use that subset for fast iteration.

The first acceptance package must include:

- the test AM from the path above;
- a KenLM model trained from `wenetspeech_lm_char.txt`;
- a binary `lm.bin`, not only an ARPA file;
- a generated `words.txt`;
- a generated `lexicon.txt`;
- an empty `output_mapping.txt` unless a mapping rule is intentionally being tested.
...
- ONNX Runtime version and archive checksum.
...
you can download Flashlight-Text with this
git clone https://github.com/flashlight/text.git
and when build the flashlight-text, it will automatically download the kenLM
so no need to download it directly.
in cmake, set this as automatically download and build with the program. 

### 9.2 ONNX Runtime local dependency

Do not rely on a bare documentation hyperlink, a system-installed ONNX Runtime, or a developer's existing environment. A clean machine may not have ONNX Runtime installed.
...
Provide a project-local ONNX Runtime root:
...
For this project, the default concrete path is:

```text
/home/jinyang_wang/Dev/ASR/ASR_wenet/wenet/SDK/0.0.3/third_party/onnxruntime
```

The `lib/` directory must contain the required shared libraries, including `libonnxruntime.so` and its versioned symlinks/files from the release package.

The SDK build should look for ONNX Runtime in this local directory by default. If a custom path is needed, it must be passed explicitly through CMake. Do not silently fall back to an unrelated system ORT unless the developer requests that behavior.

`SDK/README.md` must include a reproducible setup section similar to:

```bash
ORT_VERSION=1.25.1
SDK_ROOT=/path/to/your_project/wenet/SDK/0.0.3
...
- `SDK_ROOT` must be replaced with the user's `0.0.3` SDK project path.
- The expected final paths are `third_party/onnxruntime/include` and `third_party/onnxruntime/lib` under `SDK_ROOT`.
- The CMake option for a custom ORT root.
- How to verify that `libonnxruntime.so` exists before building.
- How runtime library lookup is handled for examples and packaged SDK binaries.
...
ONNX Runtime         shared, pinned to 1.25.1 and loaded from the SDK-provided path
...
- Do not expose ONNX Runtime headers from public SDK headers.
- Install or package the required ONNX Runtime shared objects with the SDK examples/package.
...
Exact target names must be verified against the pinned Flashlight-Text revision and ONNX Runtime package layout. Wrap external targets behind SDK-owned aliases so future dependency changes do not spread through the tree.

The ONNX Runtime CMake module must fail early with a clear error when:

- `ASR_SDK_ONNXRUNTIME_ROOT` does not exist;
- `include/onnxruntime_cxx_api.h` is missing;
- `lib/libonnxruntime.so` is missing;
- the detected version is not `1.25.1`, unless the version check is explicitly overridden for local experimentation.

### 9.5 Internal core library
...
### Reusable LM command tools

Any command used to train the LM, convert it to binary, build dictionaries, build the lexicon, or run the AM + LM acceptance test must be saved as a reusable tool under:

```text
/home/jinyang_wang/Dev/ASR/ASR_wenet/LM/kenlm_lm
```

Do not leave important one-off shell commands only in terminal history or this plan. The minimum expected layout is:
...
1. `sample_eval_metadata.py`
   - Read the 2000-line `metadata.jsonl`.
   - Deterministically select about 100 items.
   - Write the sampled metadata into `test/0.0.3`.
2. `train_kenlm_arpa.sh`
   - Train a KenLM ARPA model from the same output-unit segmentation intended for `words.txt`:
     ```text
     /home/jinyang_wang/Dev/ASR/ASR_wenet/LM/wenet_lm/training/preprocess_data/wenetspeech_lm_char.txt
     ```
...
6. `run_003_acceptance_eval.sh`
   - Run the `0.0.3` standalone decoder or SDK example on the sampled 100-item set.
   - Write raw transcripts, mapped transcripts, scores, RTF, and error summary under `test/0.0.3`.

`LM/kenlm_lm/README.md` must explain:

- required inputs and their expected paths;
- how to run each tool in order;
- where each output is written;
- how to rebuild `lm.bin`, `words.txt`, and `lexicon.txt`;
- how to run the 100-sample `0.0.3` acceptance test;
- what to check when output is empty or strange.

### Exit criteria

A package can be rebuilt from source resources without manual file editing, and the LM/training commands are preserved as reusable tools with documentation.
...
### 0.0.3 local acceptance run

After development is finished, run a local acceptance test under:

```text
/home/jinyang_wang/Dev/ASR/ASR_wenet/test/0.0.3
```

Use this metadata source:

```text
/home/jinyang_wang/Dev/ASR/ASR_wenet/data/hf_wenetspeech_test_net/wenetspeech_test_net_sample_2000/metadata.jsonl
```

Do not use all 2000 items for the first acceptance test. Use the `LM/kenlm_lm/tools/sample_eval_metadata.py` tool to choose about 100 utterances deterministically and save the sampled file, for example:

```text
test/0.0.3/metadata.sample100.jsonl
```
...
Use this LM text source to train the KenLM model:
...
1. Train KenLM under `LM/kenlm_lm`.
2. Convert the ARPA LM to binary `lm.bin`.
3. Build `words.txt`.
4. Build `lexicon.txt` for the AM tokens.
5. Assemble a self-contained `0.0.3` runtime package.
6. Decode the sampled 100 utterances with AM + LM.
7. Save transcripts, scores, logs, and summary metrics under `test/0.0.3`.

If the result is empty or strange, do not first assume the AM or dataset is broken. Treat the AM and data as valid and inspect the new implementation first:

- verify the ONNX output shape and log-score values;
- verify `tokens.txt` IDs match the AM output columns;
- verify blank, silence, and unknown IDs;
- verify the lexicon uses AM tokens exactly;
- verify `words.txt` matches the KenLM vocabulary expected by the decoder;
- verify `LexiconDecoder::decodeBegin`, `decodeStep`, and `decodeEnd` are called in the correct order;
- verify CTC frames are not collapsed or filtered before Flashlight;
- verify the package validator catches resource mismatches before decoding.
...
- The `test/0.0.3` 100-sample acceptance run completes with non-empty, plausible transcripts.
- Any empty or strange output has an implementation-level diagnosis before blaming the AM or data.
...
11. `test/0.0.3` 100-sample metadata subset.
12. Full AM + LM package built from `LM/kenlm_lm` tools.
13. Binary `lm.bin` loaded by the Flashlight/KenLM path.
14. Generated `words.txt` and `lexicon.txt` validated against the AM.
15. Empty-or-strange output diagnosis logs are available when decoding fails.
...
- The `0.0.3` package and tests do not depend on `0.0.2` runtime paths.
- A clean environment without system ONNX Runtime can build after following the README setup.
- Missing or wrong-version ONNX Runtime fails with a clear CMake error.
...
- Pin ONNX Runtime to `1.25.1`.
- Add project-local ONNX Runtime discovery through `third_party/onnxruntime` under the `0.0.3` SDK root.
- Document ONNX Runtime download/copy steps in `SDK/README.md`.
...
- `LM/kenlm_lm` reusable command tools.
- `LM/kenlm_lm/README.md`.
- Coverage reports.
...
- [ ] The finished `0.0.3` SDK builds and runs independently of the `0.0.2` directory and runtime outputs.
- [ ] ONNX Runtime is pinned to `1.25.1` and discovered from `third_party/onnxruntime` under the `0.0.3` SDK root by default.
- [ ] `SDK/README.md` explains how to download ONNX Runtime `1.25.1`, copy `include/` and `lib/`, and verify `libonnxruntime.so`.
...
- [ ] The first acceptance run uses about 100 sampled items from `metadata.jsonl`, not the full 2000-item file.
- [ ] The AM + LM acceptance test is run under `/home/jinyang_wang/Dev/ASR/ASR_wenet/test/0.0.3`.
- [ ] `lm.bin`, `words.txt`, and `lexicon.txt` are built from documented tools under `/home/jinyang_wang/Dev/ASR/ASR_wenet/LM/kenlm_lm`.
- [ ] Empty or strange AM + LM output is debugged as an implementation issue first, with logs showing the checked decoder inputs and resource IDs.
