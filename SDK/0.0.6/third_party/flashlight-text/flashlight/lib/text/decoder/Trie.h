...
// Local SDK patch: this is only a small-vector reserve hint.  A spelling may
// legitimately carry more labels (for example static words plus ambiguous
// runtime contacts), so it must never be treated as a correctness limit.
constexpr int kTrieInitialLabelReserve = 6;

enum class SmearingMode {
  NONE = 0,
  MAX = 1,
  LOGADD = 2,
  // Strict max across both terminal labels and child paths.  Kept separate
  // from legacy MAX so existing no-context decoder behavior is unchanged.
  RUNTIME_CONTACT_MAX = 3,
};
...
struct TrieNode {
  explicit TrieNode(int idx)
      : children(std::unordered_map<int, std::shared_ptr<TrieNode>>()),
        idx(idx),
        maxScore(0) {
    labels.reserve(kTrieInitialLabelReserve);
    scores.reserve(kTrieInitialLabelReserve);
  }
