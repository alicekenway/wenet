...
std::string SemanticKeyForHypothesis(const DecodedHypothesis& hyp) {
  ...
  for (const DecodedEntity& entity : hyp.entities) {
    ...
    for (const DecodedContactCandidate& candidate : entity.candidates) {
      candidate_ids.push_back(candidate.contact_id);
    }
...
std::vector<DecodedHypothesis> MergeMainAndEligibleContactHypotheses(
  ...
  for (DecodedHypothesis& hyp : contact_hyps) {
    if (HasCompletedContact(hyp)) {
      main_hyps.push_back(std::move(hyp));
    }
  }
  return DeduplicateDecodedHypotheses(std::move(main_hyps), limit,
                                      /*contact_aware=*/true);
}
