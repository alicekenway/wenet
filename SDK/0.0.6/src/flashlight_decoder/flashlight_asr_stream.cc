...
AsrResult BuildAsrResult(const std::vector<DecodedHypothesis>& hyps,
  ...
  for (const DecodedHypothesis& hyp : hyps) {
    ...
    nbest.entities.reserve(hyp.entities.size());
    for (const DecodedEntity& source : hyp.entities) {
      EntityResult entity;
      entity.type = source.type;
      entity.text = source.text;
      entity.start_ms = source.start_frame * 40.0f;
      entity.end_ms = source.end_frame * 40.0f;
      entity.score = source.score;
      entity.ambiguous = source.ambiguous;
      entity.candidates.reserve(source.candidates.size());
      for (const DecodedContactCandidate& source_candidate :
           source.candidates) {
        entity.candidates.push_back(ContactCandidateResult{
            source_candidate.contact_id, source_candidate.display_name});
      }
      nbest.entities.push_back(std::move(entity));
    }
  ...
  if (!result.nbest.empty()) {
    ...
    result.entities = result.nbest.front().entities;
  }
  if (debug) {
...
struct FlashlightAsrStream::Impl {
  Impl(std::shared_ptr<FlashlightAsrResources> shared_in,
       FlashlightDecoderResourcePtr main_decoder_resource,
       FlashlightDecoderResourcePtr contact_decoder_resource)
      : shared(std::move(shared_in)),
        ...
        main_decoder(std::make_unique<FlashlightCtcStreamDecoder>(
            std::move(main_decoder_resource))) {
    if (contact_decoder_resource) {
      contact_decoder = std::make_unique<FlashlightCtcStreamDecoder>(
          std::move(contact_decoder_resource));
    }
  }
  ...
  std::unique_ptr<FlashlightCtcStreamDecoder> main_decoder;
  std::unique_ptr<FlashlightCtcStreamDecoder> contact_decoder;
...
FlashlightAsrStream::FlashlightAsrStream(
    ...
    FlashlightDecoderResourcePtr contact_decoder_resource)
    : shared_(std::move(shared)),
      contact_decoder_resource_(std::move(contact_decoder_resource)) {}
...
Status FlashlightAsrStream::EnsureInitialized() {
  ...
  try {
    impl_ = std::make_unique<Impl>(shared_, shared_->decoder_resource,
                                   contact_decoder_resource_);
    impl_->AddDebugLog("stream_init");
    if (contact_decoder_resource_) {
      const auto& contacts = contact_decoder_resource_->DynamicContacts();
      impl_->AddDebugLog("runtime_contact_context forms=" +
                         std::to_string(contacts ? contacts->Forms().size()
                                                  : 0));
      impl_->AddDebugLog("dual_lm_contact_decoder=true");
    }
...
Status FlashlightAsrStream::ForwardCurrentWindow(bool final_padding) {
  ...
  try {
    ...
    Status status = impl_->main_decoder->DecodeChunk(
        flat.data(), static_cast<int>(log_probs.size()), info.vocab_size);
    if (!status.ok()) {
      return impl_->RecordStatus(status);
    }
    if (impl_->contact_decoder) {
      status = impl_->contact_decoder->DecodeChunk(
          flat.data(), static_cast<int>(log_probs.size()), info.vocab_size);
      if (!status.ok()) {
        return impl_->RecordStatus(status);
      }
...
Status FlashlightAsrStream::EmitPartialIfAvailable() {
  ...
  if (!impl_->contact_decoder) {
    // Preserve the historical no-context partial-result path exactly.
    auto partial_or = impl_->main_decoder->PartialResult();
    if (!partial_or.ok()) {
      return impl_->RecordStatus(partial_or.status());
    }
    hyps.push_back(std::move(partial_or).value());
  } else {
    auto main_or = impl_->main_decoder->PartialResults();
    if (!main_or.ok()) {
      return impl_->RecordStatus(main_or.status());
    }
    auto contact_or = impl_->contact_decoder->PartialResults();
    if (!contact_or.ok()) {
      return impl_->RecordStatus(contact_or.status());
    }
    hyps = MergeMainAndEligibleContactHypotheses(
        std::move(main_or).value(), std::move(contact_or).value(),
        shared_->config.nbest);
  }
  if (hyps.empty()) {
    return Status::Ok();
  }
...
Status FlashlightAsrStream::FinalizeIfReady() {
  ...
  std::vector<DecodedHypothesis> hyps;
  if (!impl_->contact_decoder) {
    auto hyps_or = impl_->main_decoder->Finalize(shared_->config.nbest);
    if (!hyps_or.ok()) {
      return impl_->RecordStatus(hyps_or.status());
    }
    hyps = std::move(hyps_or).value();
  } else {
    auto main_or = impl_->main_decoder->Finalize();
    if (!main_or.ok()) {
      return impl_->RecordStatus(main_or.status());
    }
    auto contact_or = impl_->contact_decoder->Finalize();
    if (!contact_or.ok()) {
      return impl_->RecordStatus(contact_or.status());
    }
    hyps = MergeMainAndEligibleContactHypotheses(
  ...
  impl_->AddDebugLog("final_nbest count=" + std::to_string(hyps.size()));
