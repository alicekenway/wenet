...
class FlashlightEngineImpl final : public AsrEngine {
 public:
  ...
  StatusOr<std::unique_ptr<AsrStream>> CreateStream(
    ...
    if (!compiled->ContactDecoderResource()) {
      return Status::Internal(
          "compiled decode context has no contact decoder resource");
    }
    return std::unique_ptr<AsrStream>(
        new internal::flashlight_decoder::FlashlightAsrStream(
            resources_, compiled->ContactDecoderResource()));
  }
  ...
  try {
    ...
    auto decoder_resource =
        std::make_shared<internal::flashlight_decoder::FlashlightDecoderResource>(
            ...
            package.supports_runtime_contacts ? package.contact_class_word
                                              : "",
            package.supports_runtime_contacts ? package.contact_kenlm_bin
                                              : std::filesystem::path(),
            package.supports_runtime_contacts ? package.contact_lm_weight
                                              : 1.5,
            package.supports_runtime_contacts
                ? package.contact_lm_accumulation_factor
                : 0.5,
            package.supports_runtime_contacts ? package.contact_lm_max_bonus
                                              : 0.0);
