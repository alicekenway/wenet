...
int main() {
  try {
    ...
    ExpectThrows([&]() { mapper.ValidateForPreLm(longest_path); },
                 "pre-LM validation must reject strict source prefixes");

    const fs::path pre_lm_path = dir / "pre_lm_mapping.txt";
    WriteFile(pre_lm_path,
              "A B C -> X\n"
              "C D -> Y\n");
    auto pre_lm_mapper = OutputSequenceMapper::Load(pre_lm_path, words);
    pre_lm_mapper.ValidateForPreLm(pre_lm_path);
    auto suffix = pre_lm_mapper.MatchIdSuffix({4, 1, 2, 3}, 4);
    Expect(suffix.has_value() && suffix->source_length == 3 &&
               *suffix->target == std::vector<int>({5}),
           "pre-LM suffix match failed");
    ...
    const fs::path anchored_path = dir / "anchored_mapping.txt";
    WriteFile(anchored_path,
              "^A B -> X\n"
              "C D$ -> Y\n");
    auto anchored_mapper = OutputSequenceMapper::Load(anchored_path, words);
    Expect(anchored_mapper.RewriteIds({1, 2, 3, 4}) ==
               std::vector<int>({5, 6}),
           "anchored mappings failed at their required boundaries");
    Expect(anchored_mapper.RewriteIds({4, 1, 2, 3}) ==
               std::vector<int>({4, 1, 2, 3}),
           "start-anchored mapping matched away from utterance start");
    Expect(anchored_mapper.RewriteIds({3, 4, 1}) ==
               std::vector<int>({3, 4, 1}),
           "end-anchored mapping matched away from utterance end");
