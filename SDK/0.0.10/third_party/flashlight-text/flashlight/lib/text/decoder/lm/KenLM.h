...
class FL_TEXT_API KenLM : public LM {
 public:
  ...
  // Scores KenLM's unknown vocabulary item from `state`.  This is used both
  // as the relative-score baseline and to advance an explicit OOV without
  // resetting language history.
  std::pair<LMStatePtr, float> scoreUnknown(const LMStatePtr& state,
                                           int cacheKey = -2);
